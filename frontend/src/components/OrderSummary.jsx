...
const OrderSummary = () => {
	...
	const handlePayment = async () => {
		...
		if (!import.meta.env.VITE_STRIPE_PUBLISHABLE_KEY) {
			...
			alert("Payment configuration error. Please check environment variables.");
			return;
		}
		...
		try {
			...
			if (!stripe) {
				...
				alert("Failed to load payment processor. Please check your internet connection.");
				return;
			}
			...
			if (error.response) {
				...
				alert(`Server error: ${error.response.data.message || error.response.data.error}`);
